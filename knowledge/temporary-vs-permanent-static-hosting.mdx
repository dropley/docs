...
Dropley is for the first case. It publishes finished static files to a public URL, then deletes the artifact when its selected lifetime ends. It is not a way to run an ongoing application.
...
| Share anything sensitive | Do not use a public Dropley link | Anyone with the URL can open it until it expires. |
...
The countdown starts when publishing succeeds. When it ends, Dropley permanently deletes the artifact and its files; the old URL no longer works. You cannot extend an expired artifact, so upload the finished files again when you need a new link.
